...
const {
  ...
  VIMEO_USER_ID,
  VIMEO_ACCESS_TOKEN,
  VIMEO_LEERJAREN_INLEIDINGEN_PROJECT_ID,
} = process.env;
...
export async function GET(
  ...
  // fetch all intro lessons folders first
  const allItemsInFolderResult = await fetch(
    `${VIMEO_API_URL}/users/${VIMEO_USER_ID}/projects/${VIMEO_LEERJAREN_INLEIDINGEN_PROJECT_ID}/items?fields=folder.name,folder.uri,folder.metadata.connections.ancestor_path,folder.metadata.connections.videos`,
    {
      headers: {
  ...
  return Response.json(videosData.data[0]);
}
